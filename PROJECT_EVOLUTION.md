...
**Class One Rapids** is an experimental project answering the question: *"How to create an agentic, spec-driven development system using modern context engineering techniques?"* The project evolved from September 2025 through October 2025 as a meta-development exercise—using Claude Code to build both:
...
**Critical Learning**: The attributes system implementation failure revealed that *when* and *where* architectural guidance appears matters more than *what* it says.
...
### General Issues (from README)
1. **Folder Organization**: By-feature organization not common for small React apps
2. **Starting Projects**: Agent tends to overthink initial setup
3. **Config Management**: Tries to create "standard configs" instead of building incrementally
4. **MVP Focus**: Too focused on marketing/metrics/full testing for POC-level work
5. **Tooling Knowledge**: Not aware of modern Expo defaults (babel-preset-expo)
6. **Jest Compatibility**: Ongoing challenges
7. **Research Bloat**: Too much research can overwhelm context
8. **Code Refactoring**: Tends toward weird folder structures
9. **Architecture Patterns**: Prefers services over hooks, classes over functions
10. **Performance Requirements**: Sometimes inappropriately strict for MVPs
...
**Class One Rapids** demonstrates a successful experiment in meta-development: using AI-assisted tools to build better AI-assisted development workflows. The key insight: the *system* is the product, not the game—though the game provides essential validation.
