...
- Tests not ideal?
- commit code generated along with changes that resulted in that code, so git history can be used by an analyzer in a pinch
...
- "red" phase of TDD needs to be more clear, its considering failure on some jest setup stuff to be be good for that phase. It should be failing because the implementation isn't complete yet. At least at this stage. Though it  
...
# Some prompts I want to run
- Track progress of the tasks in the tasks list generated by @.claude/commands/flow/tasks.md 
- Create a copy of execute-task that uses git worktree and a parent directory above class-one-rapids so that I can run two flows at the same time.
